...
class Data:
    ...
    linear_tcb_start_ns: int = 0
    """
    Binning start (nanoseconds), takes effect on next run start.
    """

    @property
    def linear_tcb_start_us(self) -> float:
        """Binning start (microseconds), takes effect on next run start."""
        return self.linear_tcb_start_ns / 1000.0

    @linear_tcb_start_us.setter
    def linear_tcb_start_us(self, value: float) -> None:
        self.linear_tcb_start_ns = round(value * 1000.0)

    linear_tcb_end_ns: int = 100_000_000
    """
    Binning end (nanoseconds), takes effect on next run start.
    """

    @property
    def linear_tcb_end_us(self) -> float:
        """Binning end (microseconds), takes effect on next run start."""
        return self.linear_tcb_end_ns / 1000.0

    @linear_tcb_end_us.setter
    def linear_tcb_end_us(self, value: float) -> None:
        self.linear_tcb_end_ns = round(value * 1000.0)

    linear_tcb_num: int = 500
    """
    Number of time-bins to generate between `linear_tcb_start_ns` and `linear_tcb_end_ns`.
